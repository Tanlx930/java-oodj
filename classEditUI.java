...
public class classEditUI extends JDialog {
    ...
    public classEditUI(JFrame parent, int classNo, String module, String className, String mode, 
                       String date, String weekday, String startTime, String endTime) {
        ...
        JLabel titleLabel = new JLabel("Edit Class #" + classNo);
        ...
        for (String mod : repo.getUniqueModuleAbbreviations()) {
            moduleField.addItem(mod);
        }
        ...
        for (String mod : commonModules) {
            ...
            for (int i = 0; i < moduleField.getItemCount(); i++) {
                if (mod.equals(moduleField.getItemAt(i))) {
                    exists = true;
                    break;
                }
            }
        ...
        moduleField.setSelectedItem(module);
        formPanel.add(createFieldPanel("Module:", moduleField));
        formPanel.add(Box.createVerticalStrut(12));

        // Class Name
        classNameField = new JTextField(className, 20);
        formPanel.add(createFieldPanel("Class Name:", classNameField));
        formPanel.add(Box.createVerticalStrut(12));

        // Class Mode dropdown
        classModeField = new JComboBox<>(CLASS_MODES);
        classModeField.setSelectedItem(mode);
        formPanel.add(createFieldPanel("Class Mode:", classModeField));
        formPanel.add(Box.createVerticalStrut(12));

        // Class Date
        classDateField = new JTextField(date, 20);
        classDateField.setToolTipText("Enter date in format: YYYY-MM-DD (e.g., 2026-01-19)");
        formPanel.add(createFieldPanel("Class Date (YYYY-MM-DD):", classDateField));
        formPanel.add(Box.createVerticalStrut(12));

        // Weekday dropdown
        weekdayField = new JComboBox<>(WEEKDAYS);
        weekdayField.setSelectedItem(weekday);
        ...
        // Start Time dropdown
        startTimeField = new JComboBox<>(TIME_SLOTS);
        startTimeField.setSelectedItem(startTime);
        formPanel.add(createFieldPanel("Start Time:", startTimeField));
        formPanel.add(Box.createVerticalStrut(12));

        // End Time dropdown
        endTimeField = new JComboBox<>(TIME_SLOTS);
        endTimeField.setSelectedItem(endTime);
    ...
    public String getModuleAbbreviation() {
        return (String) moduleField.getSelectedItem();
    }
    ...
    public String getClassMode() {
        return (String) classModeField.getSelectedItem();
    }

    public String getClassDate() {
        return classDateField.getText().trim();
    }

    public String getWeekday() {
        return (String) weekdayField.getSelectedItem();
    }

    public String getClassStartTime() {
        return (String) startTimeField.getSelectedItem();
    }

    public String getClassEndTime() {
        return (String) endTimeField.getSelectedItem();
    }
}